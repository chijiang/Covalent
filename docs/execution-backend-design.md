...
| **1 — Docker backend** | `DockerBackend` (incl. former `SandboxClient`), `Dockerfile.sandbox`, CI image build, labels, reaper, GC. Skills + scripts via `backend.spawn_stream`/`exec` (exec-stdio spike **passed**, see Process Execution). Workspace tools unchanged (host path). | Docker backend selectable; adversarial skill contained; FS still works. |
