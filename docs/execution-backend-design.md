...
| **1 — Docker backend** | Split into **1a (done)** and **1b (pending)**. **1a**: `DockerBackend` + `DockerExecProcess` (asyncio-subprocess look-alike over a hijacked exec socket; `SkillProcessHandle` unchanged), `Dockerfile.sandbox`, `session_id` threaded through `acquire`/`_spawn` (pool keyed per `(skill, session)`), `run_skill_script` via `backend.exec`, lazy per-session container bind-mounting skill source dirs + workspace at host-absolute paths, command rewrite (`sys.executable`→`python`, runner path→`/runners/`). Verified: 69 tests green (FS unchanged) + real-container smoke (`script/smoke-docker-skillhandle.py`) drives a live `SkillProcessHandle` over Docker exec. **1b**: reaper/GC, resource limits, network profile, per-session teardown, adversarial suite, image CI, in-container `kill` for hung execs, script stdin. Workspace tools unchanged (host path). | 1a: Docker backend selectable; skill runner + script execute in a container; FS still works. 1b: production hardening. |
| **2 — Workspace refactor** | Route workspace tools through `WorkspaceAccess` methods (works for `host_path is None`). FS/Docker still pathlib-backed. Resolve async bridge. | Tools fully backend-agnostic; in-container file ops possible. |
...
- **Unit:** each backend against a fake client (no daemon/cluster). `make_backend`
  factory, `ensure` idempotency, `exec` timeout, `stop`/teardown, orphan listing.
- **Parameterized suite:** the existing `tests/test_workspace_tools.py`, skill,
  and public-invoke tests run against **each** available backend (FS always;
  Docker/k8s behind a `@pytest.mark.docker` / `@pytest.mark.k8s` gate when the
  environment provides them). This is the regression contract: identical agent
  behavior across backends.
- **Adversarial (Phase 4):** a fixture skill attempting outbound network,
  fork-bomb, and path escape — each contained within budget on Docker and k8s.
- **Lifecycle:** kill backend mid-session, restart, assert reaper reclaims
  orphaned sandbox resources.
