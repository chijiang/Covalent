...
def _write_download_bytes(base_root: Path, scope_id: str, data: bytes, safe_name: str) -> Path:
    target_dir = _download_session_dir(base_root, scope_id)
    target_dir.mkdir(parents=True, exist_ok=True)
    target_path = _next_available_path(target_dir, safe_name)
...
def _build_download_payload(
    ...
    *,
    content_type: str,
    summary: str,
    download_base_path: str = "/api/backend/downloads",
) -> dict[str, Any]:
...
def _save_download_artifact(
    ...
    *,
    ...
    """Persist in-memory bytes as a downloadable artifact and return the same
    payload shape as publish_downloadable_file (download_url included)."""
    base_root = settings.workspace_root()
    safe_name = _safe_download_filename(download_name, "download")
    target_path = _write_download_bytes(base_root, scope_id, data, safe_name)
    return _build_download_payload(base_root, scope_id, target_path, content_type=content_type, summary=summary)
